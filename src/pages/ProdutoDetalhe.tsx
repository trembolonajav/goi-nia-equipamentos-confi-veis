...
export default function ProdutoDetalhe() {
  ...
  return <main className="page" style={{ maxWidth: 1300 }}>
    ...
    <div className="dash-cols" style={{ marginTop: 24, display: "grid", gridTemplateColumns: "1fr 340px", gap: 24, alignItems: "start" }}>
      <section className="card"><h2 className="h2">Patrimônios</h2><p className="section-note">Cada unidade física tem estado e localização próprios.</p><div className="list">{pats.map(p => <div key={p.cod} className="card-tight" style={{ display: "grid", gridTemplateColumns: "110px 1fr auto", gap: 14, alignItems: "center" }}><span className="mono orange">{p.cod}</span><span><span style={{ display: "block" }}>{p.local || "Local não informado"}</span><small className="muted">{p.serie || "Série não informada"}</small></span><Tag cor={COR_ESTADO[p.estado] || "var(--muted)"}>{p.estado}</Tag></div>)}{pats.length === 0 && <div className="empty">Nenhum patrimônio vinculado.</div>}</div></section>
      <aside className="stack"><div className="card"><h2 className="h2">Cadastro comercial</h2>{[["Código", pr.id], ["Unidade", pr.unidadeLocacao || "UNIDADE"], ["Valor de reposição", brl.format(Number(pr.reposicao || 0))], ["Período mínimo", pr.minimo || "1 diária"]].map(([r,v]) => <div className="spread" style={{ marginTop: 10 }} key={r}><span className="muted">{r}</span><span>{v}</span></div>)}</div><div className="card"><h2 className="h2">Indicadores operacionais</h2><div className="spread"><span className="muted">Taxa de ocupação</span><span>{ocupacao}%</span></div></div></aside>
    </div>
  </main>;
}

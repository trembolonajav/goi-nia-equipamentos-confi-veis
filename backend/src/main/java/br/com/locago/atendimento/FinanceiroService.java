...
public class FinanceiroService {
  ...
  public Map<String,Object> resumo(){
    Map<String,BigDecimal> caixa=jdbc.sql("select coalesce(sum(case when tipo='ENTRADA' then valor when tipo='SAIDA' then -valor else 0 end),0) movimento,coalesce(sum(case when tipo='ENTRADA' then valor else 0 end),0) entradas,coalesce(sum(case when tipo='SAIDA' then valor else 0 end),0) saidas from lancamento_financeiro where status='PAGO'")
      .query((r,n)->Map.of("movimento",r.getBigDecimal("movimento"),"entradas",r.getBigDecimal("entradas"),"saidas",r.getBigDecimal("saidas"))).single();
    BigDecimal inicial=jdbc.sql("select coalesce(sum(saldo_inicial),0) from conta_financeira where ativo").query(BigDecimal.class).single();
    BigDecimal receber=jdbc.sql("select coalesce(sum(greatest(valor-recebido,0)),0) from cobranca_atendimento where cancelada_em is null and status<>'PAGA'").query(BigDecimal.class).single();
    BigDecimal pagar=jdbc.sql("select coalesce(sum(saldo),0) from conta_pagar where cancelada_em is null and status<>'PAGA'").query(BigDecimal.class).single();
    return Map.of("saldo",inicial.add(caixa.get("movimento")),"entradas",caixa.get("entradas"),"saidas",caixa.get("saidas"),"aReceber",receber,"aPagar",pagar);
  }
